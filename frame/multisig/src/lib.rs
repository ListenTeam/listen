...
type BalanceOf<T> = <<T as Trait>::MulsigCurrency as Currency<<T as frame_system::Trait>::AccountId>>::Balance;
...
pub trait Trait: frame_system::Trait {
	...
	type MulsigCurrency: ReservableCurrency<Self::AccountId>;
...
decl_module! {
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		fn cancel_as_multi(origin,
			...
			let _ = T::MulsigCurrency::unreserve(&m.depositor, m.deposit);
...
impl<T: Trait> Module<T> {
	...
	fn operate(
		...
		if let Some(mut m) = <Multisigs<T>>::get(&id, call_hash) {
			...
			if let Some((call, call_len)) = maybe_approved_call {
				...
				<Multisigs<T>>::remove(&id, call_hash);
				Self::clear_call(&call_hash);
				T::MulsigCurrency::unreserve(&m.depositor, m.deposit);
			...
			let stored = if let Some(data) = maybe_call.filter(|_| store) {
				...
				T::MulsigCurrency::reserve(&who, deposit)?;
				false
			};
	...
	fn clear_call(hash: &[u8; 32]) {
		if let Some((_, who, deposit)) = Calls::<T>::take(hash) {
			T::MulsigCurrency::unreserve(&who, deposit);
		}
	}
