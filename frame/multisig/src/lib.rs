...
// 			*****************************************************************************************
			...
			if let Some(mut m) = <Multisigs<T>>::get(&id, call_hash) {
				...
				if let Err(pos) = m.approvals.binary_search(&who) {
					...
					// 如果；自己已经在赞同的队列里 并且赞同票小于threshould 那么直接报错结束
				} else {
...
// 			**************************************************************************************************
			// 如果这个多签原本不存在
				...
				// 如果threshould大于1 那么就添加信息 如果等于1 直接执行
				if threshold > 1 {
		...
		fn approve_as_multi(origin,
			threshold: u16,
			other_signatories: Vec<T::AccountId>,  // 这个多签是所有人的账号
			maybe_timepoint: Option<Timepoint<T::BlockNumber>>,
			call_hash: [u8; 32],
		) -> DispatchResult {
			let who = ensure_signed(origin)?;
			...
			// 说明自己投赞成票的时候参数不是那些人组成的多签id 而是那些人
			let id = Self::multi_account_id(&signatories, threshold);
			...
			if let Some(mut m) = <Multisigs<T>>::get(&id, call_hash) {
				...
				// 这里跟上一个方法的唯一区别就是不会创建一个可以直接执行的方法
				 else {
