...
//	type ShouldSubOrigin: OnUnbalanced<NegativeImbalanceOf<Self>>;
	...
	// transx基金会
	type ListenFoundation: EnsureOrigin<Self::Origin, Success=Self::AccountId>;
...
pub trait Subtrait: Trait + system::Trait{
...
// 		}
		...
		fn foundation_tag_for_burn(origin, who: T::AccountId, asset_id: T::AssetId) -> DispatchResult{
			// 只有基金会成员才有权限
			let origin = T::ListenFoundation::ensure_origin(origin)?;
...
impl<T: Subtrait> frame_system::Trait for ElevatedTrait<T> {
	...
	type MaximumExtrinsicWeight = T::MaximumExtrinsicWeight;
...
//	type ShouldSubOrigin = T::ShouldSubOrigin;
	...
	type ListenFoundation = T::ListenFoundation;
