...
parameter_types! {
	pub const ListenFoundationMotionDuration: BlockNumber = 5 * DAYS;
	pub const TransxMaxProposals: u32 = 100;
}
// transx基金会
type ListenFoundation = pallet_collective::Instance3;
impl pallet_collective::Trait<ListenFoundation> for Runtime {  // 这个写法很特殊

	type Origin = Origin;
	type Proposal = Call;
	type Event = Event;
	// 现在的议会多了一个时间参数
	type MotionDuration = ListenFoundationMotionDuration;
	type MaxProposals = TransxMaxProposals;
}
...
impl generic_asset::Trait for Runtime{
	...
	// Listen基金会
	type ListenFoundation = pallet_collective::EnsureMember<AccountId, ListenFoundation>;
